 ...
 *
 * <br>
 * The classloaders in the searchlist are of two types: <b>shared</b> and
 * <b>non-shared</b>. A shared classloader may be in use by other code, and so
 * <i>no</i> duplicates should be made of the classes in the loaders. <br>
 * A non-shared classloader is private to this SearchlistClassLoader, and so
 * there is no possibility that other code could be using them. To avoid
 ...
 * ensures the JVM can find the correct loader when loading associated classes
 * (including shared classes).
 *
 * <br>
 * The SearchlistClassLoader API therefore makes a clear distinction between
 * <b>shared</b> and <b>non-shared</b> classloaders. <br>
 * The {@link #add(ClassLoader)} method adds an <i>existing</i> classloader
 * which means the added classloader is treated as being <i>shared</i>. <br>
 * The {@link #add(URL)} method adds a new internally created classloader which
 * loads the content associated with the specified URL, which means the
 * internally created classloader is <i>non-shared</i>. <br>
 *
 * <br>
 * SearchlistClassLoader therefore also allows control over the order in which
 * classloaders are searched, through the {@link #setSearchMode(byte)} method.
 *
 * <br>
 * The possible <i>searchmodes</i> are:
 * <ul>
 * <li>SHARED <br>
 * added classloaders are searched before added URLs, which causes an existing
 * class instance to be used (and SHARED) in preference to loading (or creating)
 * a NON-SHARED duplicate.
 *
 * <li>NONSHARED <br>
 * added URLs are searched <i>before</i> added classloaders. This will create a
 * NON-SHARED copy of a class (ie a duplicate) in preference to using a SHARED
 * one from another classloader.
 *
 * <li>ORDERED <br>
 * added classloaders and URLs are searched in the order in which they were
 * added to the searchlist.
 * </ul>
 *
 * <br>
 * There is also a method which retrieves a class <i>without</i> searching any
 * added classloaders. This effectively retrieves the <i>canonical</i> instance
 * of the requested class (see {@link #loadLocalClass(String)} and
 * {@link #getLocalResource(String)}).
 *
 * <p>
 * <i>Implementation notes:</i>.
 *
 * <br>
 * Because each class object is associated with the classloader which defined it
 * (see ClassLoader.defineClass(...)), SearchlistClassLoader must associate
 * <i>itself</i> with <i>all</i> class objects it loads through
 * <i>non-shared</i> loaders, and similarly <i>must not</i> associate itself
 * with class objects loaded through shared loaders. (See
 * {@link #findClass(String)}.)
 *
 * <br>
 * The structure of the internal ClassLoader methods is as per the instructions
 * in {@link java.lang.ClassLoader}. While I don't think this is necessary any
 * longer, it was quite easy to comply with the instructions.
 */
public class SearchlistClassLoader extends ClassLoader {
	 ...
	 *
	 * @param mode
	 *            enum for the searchmode: SHARED, NONSHARED, ORDERED
	 */
	public void setSearchMode(byte mode) {
		...
		if (searchMode <= 0 || searchMode > ORDERED) {
			System.out.println("SearchlistClassLoader.setSearchMode: " + "Invalid search mode: " + mode
	 ...
	 *
	 * @param loader
	 *            the ClassLoader to add to the searchlist.
	 */
	 ...
	 *
	 * Creates a new URLClassLoader and adds it to the searchlist as a non-shared
	 * classloader.
	 *
	 * @param url
	 *            the URL to add to the searchlist.
	 */
	 ...
	 */
	public URL[] getURLs() {
		return (content != null ? ((URLClassLoader) content.loader).getURLs() : EMPTY_URL);
	}
	 ...
	 *
	 * <br>
	 * This does <i>not</i> search the searchlist. Only classes loaded directly by
	 * this loader or its parent are returned.
	 *
	 * <br>
	 * This method can be used to retrieve the <i>canonical</i> instance of a class.
	 * If this method is called on a set of SearchlistClassLoaders, then the only
	 * classloader which will return the class is the one which originally loaded it
	 * (assuming no duplicates have been created yet).
	 *
	 * @param name
	 ...
	 *
	 * <br>
	 * This does <i>not</i> search the searchlist. Only resources loaded directly by
	 * this loader or its parent are returned.
	 *
	 * <br>
	 * This method can be used to retrieve the <i>canonical</i> URL for a resource.
	 * If this method is called on a set of SearchlistClassLoaders, then the only
	 * classloader which will return the resource is the one which originally loaded
	 * it (assuming no duplicates have been created yet).
	 *
	 * @param name
	 ...
	 *
	 *            Traverses the searchlist looking for a classloader which can
	 *            return the specified class.
	 *
	 *            <br>
	 * 			This method is called by inherited loadClass() methods whenever a
	 *            class cannot be found in the parent classloader.
	 *
	 *            <br>
	 * 			If the class is found using a <i>shared</i> loader, then it is
	 ...
	 *            SearchlistClassLoader, which causes Java to associate the new
	 *            class object with the SearchlistClassLaoder, rather then the
	 *            non-shared loader.
	 *
	 * @param name
	 *            the fully-qualified name of the class
	 ...
	 *
	 *            <em>Note</em>Inherited behaviour of loadClass() and getResource()
	 *            means that this method is <em>not</em> called if our parent
	 *            classloader has already found the data.
	 *
	 * @param path
	 ...
	 *
	 * If the library is not found on the searchpath, then <i>null</i> is returned,
	 * indicating to the Java machine that it should search java.library.path.
	 *
	 * @param libname
	...
	/**
	 * return the correct classloader for the specified position in the search.
	 *
	 * @param index
	 *            the position (step) in the search process
	 * @param mode
	...
	protected Loader getLoader(int index, byte mode) {
		...
		// return non-shared loaders before shared loaders
		{
			...
			result = (Loader) (pos < search.size() ? search.get(pos) : search.get(pos - divide));
		}
			break;
	 ...
	 * @param match
	 *            the list of chars to match, in a string.
	 * @param replace
	 *            the list of corresponding chars to replace matched chars with.
	 *
	 *            <pre>
	 ...
	 *
	 * The containing SearchlistClassLoader needs to know information about each
	 * loader in the list.
	 */
