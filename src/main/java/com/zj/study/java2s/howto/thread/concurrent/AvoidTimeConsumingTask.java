package com.zj.study.java2s.howto.thread.concurrent;
...
public class AvoidTimeConsumingTask {
	final int CPU_COUNT = 4;
	BlockingQueue<Runnable> lightTaskQueue = new LinkedBlockingDeque<Runnable>();

	ThreadPoolExecutor lightExecutor = new ThreadPoolExecutor(CPU_COUNT, CPU_COUNT, 60L, TimeUnit.SECONDS,
			lightTaskQueue);

	BlockingQueue<Runnable> heavyTaskQueue = new LinkedBlockingDeque<Runnable>();

	ThreadPoolExecutor heavyExecutor = new ThreadPoolExecutor(1, 1, 60L, TimeUnit.SECONDS, heavyTaskQueue);
	...
	public static void main(String[] args) throws Exception {
		AvoidTimeConsumingTask q = new AvoidTimeConsumingTask();
...
class SampleHeavyTask implements Runnable {
	...
	public void run() {
		try {
			AvoidTimeConsumingTask.heavyTaskRunning.set(true);
			...
			AvoidTimeConsumingTask.heavyTaskRunning.set(false);
		}
	}
}