package com.zj.study.framework.netty.websocket.server;
 ...
 */
public class ProcesssWsFrameHandler extends SimpleChannelInboundHandler<WebSocketFrame> {
	...
	private static final Logger logger = LoggerFactory.getLogger(ProcesssWsFrameHandler.class);
	...
	protected void channelRead0(ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
		...
		if (frame instanceof TextWebSocketFrame) {
			...
			/* 群发实现，Mark增加，一对一道理一样 */
			group.writeAndFlush(
					new TextWebSocketFrame("Client " + ctx.channel() + " say:" + request.toUpperCase(Locale.CHINA)));
		} else {
