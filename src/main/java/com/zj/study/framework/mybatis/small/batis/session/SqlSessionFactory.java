...
public class SqlSessionFactory {
	...
	public SqlSessionFactory() throws IOException, DocumentException {
		// 加载数据库信息
		loadDbInfo();
	...
	private void loadDbInfo() throws IOException {
		...
		conf.setDbDriver(p.get("jdbc_driver").toString());
		conf.setDbPassword(p.get("jdbc_password").toString());
		conf.setDbUrl(p.get("jdbc_url").toString());
		conf.setDbUserName(p.get("jdbc_username").toString());
	}

	private void loadMappersInfo() throws DocumentException {
		URL resources = null;
